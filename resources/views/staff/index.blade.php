...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form method="GET" action="{{ url('/staff') }}" accept-charset="UTF-8" class="" role="search">
                    ...
                    <tbody>
                        @foreach ($staff as $item)
                            <tr>
                                ...
                                <td>{{ $item->address }}</td>
                                <td>
                                    <a href="{{ url('/staff/' . $item->id) }}" title="View Staff"><button
                                            class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i>
                                            View</button></a>
                                    <a href="{{ url('/staff/' . $item->id . '/edit') }}" title="Edit Staff"><button
                                            class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o"
                                                aria-hidden="true"></i> Edit</button></a>

                                    <form method="POST" action="{{ url('/staff' . '/' . $item->id) }}"
